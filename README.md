...
- A/B loop controls in the upper-right corner: each point appears on the timeline as soon as it is set, with a best-effort captured frame or site-provided sprite/VTT thumbnail above it. The translucent orange interval bar appears only when both points form an active loop; clear either point to stop looping and continue from the current position.
- Press and hold the video surface without moving to temporarily accelerate playback. UI controls are excluded from the gesture area.
- While holding, drag right to vary the temporary fast-forward rate by distance: 3x near the starting point, 5x at medium distance, and 8x farther away. Drag left to preview a backward time offset; release to seek backward once without repeatedly reloading HLS segments.
